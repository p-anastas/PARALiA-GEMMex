...
int main(const int argc, const char *argv[]) {
  ...
  int ctr = 1, samples, dev_id, dev_count;
	...
	maxDim = std::min(CoCoGetMaxDimSqAsset2D(3, sizeof(double), STEP_TRANS, loc_dest),
	std::min(CoCoGetMaxDimSqAsset2D(3, sizeof(double), STEP_TRANS, loc_src), (long int) MAX_DIM_TRANS));
  ...
  //Only model pinned memory transfers loc_src host loc_dest dev and visa versa
	...
	for(short dev_id_idx = 0 ; dev_id_idx < LOC_NUM; dev_id_idx++){
  	...
  	unit_buffs[2*dev_id_idx] = CoCoMalloc(maxDim*(maxDim+1)*elemSize, deidxize(dev_id_idx), 1);
		unit_buffs[2*dev_id_idx+1] = CoCoMalloc(maxDim*(maxDim+1)*elemSize, deidxize(dev_id_idx), 1);
		timer = csecond() - timer;
  ...
  CQueue_p transfer_queue_list[LOC_NUM][LOC_NUM] = {{NULL}};
	...
	double transfer_t_bid_sum, transfer_t_bid_mean, error_margin_bid, t_lat = 0;
	long int sample_sz, sample_sz_bid;
	CoCoPeLiaSelectDevice(loc_dest);
	Event_timer_p device_timer = new Event_timer(loc_dest);
	for (dim = 1; dim < MAX_DIM_TRANS; dim+=step){ // maxDim+1
		if (dim == step + 1) dim = minDim;
		...
		if(dim == 1) t_lat = transfer_t_mean;
		CoCoSyncCheckErr();
		report_run(filename, dim, dim , transfer_t_mean, error_margin, sample_sz, bench_t);
	}
	fprintf(stderr,"----------------------------------------------------------------------\n");

	fprintf(stderr,"Latency estimated by 1-byte transfer :\t\t t_lat = %e ms\n", t_lat*1000);
	double t_b = transfer_t_mean - t_lat;
	dim -= step;
	fprintf(stderr,"BW estimated by 8-bytes X (%d X %d):\t t_b = %e ms ( %lf Gb/s)\n", dim, dim, t_b*1000, Gval_per_s(dim*dim*8, t_b));
	int byte_breakpoint = (int) (t_lat/t_b*(dim*dim*8)); 
	fprintf(stderr,"Latency-Bandwidth breakpoint:\t\t\t b_eq = %d bytes [8-bytes X (%d X %d)]\n", byte_breakpoint, (int) sqrt(byte_breakpoint/8), (int) sqrt(byte_breakpoint/8));
	fprintf(stderr,"75% Bandwidth breakpoint:\t\t\t b_75 = %d bytes [8-bytes X (%d X %d)]\n", byte_breakpoint*2, (int) sqrt(byte_breakpoint/4), (int) sqrt(byte_breakpoint/4));
	fprintf(stderr,"95% Bandwidth breakpoint:\t\t\t b_95 = %d bytes [8-bytes X (%d X %d)]\n", byte_breakpoint*20, (int) sqrt(byte_breakpoint*20/8), (int) sqrt(byte_breakpoint*20/8));
	...
	for(short dev_id_idx = 0 ; dev_id_idx < LOC_NUM; dev_id_idx++){
		for(short dev_id_idy = 0 ; dev_id_idy < LOC_NUM; dev_id_idy++){
			...
			report_run(filename_over, deidxize(dev_id_idx), deidxize(dev_id_idy), transfer_t_mean, transfer_timer, MAX_ASSUMED_OTHER_LINK_TIMES_FASTER, bench_t);
		}
	}
	...
	for(short dev_id_idx = 0 ; dev_id_idx < LOC_NUM; dev_id_idx++){
		double timer = csecond();
	  CoCoFree(unit_buffs[2*dev_id_idx], maxDim*(maxDim+1)*elemSize, deidxize(dev_id_idx));
	  CoCoFree(unit_buffs[2*dev_id_idx+1], maxDim*(maxDim+1)*elemSize, deidxize(dev_id_idx));
		timer = csecond() - timer;
