...
void ManageCachesHgemm(PMD_p local_PMD){
	for(int loc = 0; loc < CHL_MEMLOCS; loc++) if(local_PMD->autotuner->Block_num[loc]){
		...
		if(local_PMD->autotuner->Block_num[loc] == -42) 
			error("PARALiAHgemm: local_PMD->autotuner->Block_num[%d] is -42\n", loc);
#endif
		...
		__half* B, long int ldB, __half beta, __half* C, long int ldC)
{
	...
	if(!reuse_problem_flag){
		...
		if (predef_controller_hgemm && local_PMD->autotuner->diff_intialized_params_ATC(predef_controller_hgemm))
			local_PMD->autotuner->mimic_ATC(predef_controller_hgemm);
