...
ATC_p predef_controller_hgemm = NULL;

#ifdef STEST
double gemm_entry_ts;
#endif

void ManageCachesHgemm(PMD_p local_PMD, int aut_idx){
	for(int loc = 0; loc < CHL_MEMLOCS; loc++) if(local_PMD->autotuner[aut_idx]->Block_num[loc]){
		...
		if(local_PMD->autotuner[aut_idx]->Block_num[loc] == -42) 
			error("PARALiAHgemm: local_PMD->autotuner[aut_idx]->Block_num[%d] is -42\n", loc);
#endif
		...
		if(current_SAB[loc] == NULL) current_SAB[loc] = 
			...
			error("PARALiAHgemm:ManageCachesHgemm -> PARALiA 3.0 should not enter this\n");
#ifdef DEBUG
			fprintf(stderr, "PARALiAHgemm: Previous Cache smaller than requested:\
		...
		if(current_SAB[loc]!= NULL) 
			error("PARALiAHgemm: current_SAB[%d] was not NULL with reuse disabled\n", loc);
		current_SAB[loc] = new Buffer(loc, local_PMD->autotuner[aut_idx]->Block_num[loc], local_PMD->autotuner[aut_idx]->Block_sz);
#endif
	}
...
void CreateTasksHgemm(PMD_p local_PMD, int aut_idx){

#ifdef DEBUG
	fprintf(stderr, "|-----> CreateTasksHgemm(%p,%ld,%ld)\n",
...
#endif
	gemm_backend_in* initial_hgemm = (gemm_backend_in*) local_PMD->problem_wrap;
	//int current_ctr = 0;
	for (int mi = 0; mi < local_PMD->decom[0]->GridSz1; mi++){
		for (int ni = 0; ni < local_PMD->decom[1]->GridSz2; ni++){
			...
			C_tile->reduce_mult = initial_hgemm->beta; 
			C_tile->W_op_name = "MM_FP16";
			...
			for (int ki = 0; ki < C_tile->W_op_num; ki++){
				...
				ptr_ker_translate->TransA = initial_hgemm->TransA;
				ptr_ker_translate->TransB = initial_hgemm->TransB;
...
void UpdateTasksHgemm(PMD_p local_PMD){
	gemm_backend_in* initial_hgemm = (gemm_backend_in*) local_PMD->problem_wrap;
	//int current_ctr = 0;
	for (int mi = 0; mi < local_PMD->decom[0]->GridSz1; mi++){
		for (int ni = 0; ni < local_PMD->decom[1]->GridSz2; ni++){
...
/// A hgemm wrapper including auto-tuning of T and cache_size, as well as device management
ATC_p PARALiAHgemm(char TransA,  char TransB, long int M, long int N, long int K, __half alpha, __half* A, long int ldA,
		__half* B, long int ldB, __half beta, __half* C, long int ldC)
{
	short lvl = 1;
#ifdef DEBUG
	fprintf(stderr, "|-----> PARALiAHgemm(%c,%c,%zu,%zu,%zu,%lf,A=%p(%d),%zu,B=%p(%d),%zu,%lf,C=%p(%d),%zu)\n",
	...
	gemm_backend_in* initial_hgemm = NULL;

	for(int cache_entries = 0; cache_entries < PMD_cache_entries; cache_entries++)
	if(PMD_cache[cache_entries] && !strcmp(PMD_cache[cache_entries]->problem_name, "MM_FP16")){
			initial_hgemm = (gemm_backend_in*) PMD_cache[cache_entries]->problem_wrap;
#ifdef DEBUG
			PMD_cache[cache_entries]->print();
#endif
			reuse_problem_flag = 1; 
			if(initial_hgemm->TransA != TransA)
			reuse_problem_flag = 0;
			if(initial_hgemm->TransB != TransB)
				reuse_problem_flag = 0;
...
#ifndef PRODUCTION
				warning("PARALiAHgemm - problem cache full, removing first entry\n");
#endif
				delete PMD_cache[0];
	...
	initial_hgemm = (gemm_backend_in*) local_PMD->problem_wrap;
	initial_hgemm->TransA = TransA;
	initial_hgemm->TransB = TransB;
	initial_hgemm->M = M;
	initial_hgemm->N = N;
	initial_hgemm->K = K;
	initial_hgemm->A = (void**) &A;
	initial_hgemm->B = (void**) &B;
	initial_hgemm->C = (void**) &C;
	initial_hgemm->alpha = &alpha;
	initial_hgemm->beta = &beta;
	initial_hgemm->ldA = ldA;
	initial_hgemm->ldB = ldB;
	initial_hgemm->ldC = ldC;
	initial_hgemm->dev_id = -1;
	...
	if(!reuse_problem_flag){
		local_PMD->problem_name = "MM_FP16";
		local_PMD->decom_num = 3;
		local_PMD->decom[0] = new Decom2D( (void*) A, M, K, ldA, TransA, HALF);
		local_PMD->decom[1] = new Decom2D( (void*) B, K, N, ldB, TransB, HALF);
		local_PMD->decom[2] = new Decom2D( (void*) C, M, N, ldC, 'N', HALF);
		...
		if (predef_controller_hgemm && local_PMD->autotuner[curr_autotuner_ctr]->diff_intialized_params_ATC(predef_controller_hgemm))
			local_PMD->autotuner[curr_autotuner_ctr]->mimic_ATC(predef_controller_hgemm);
		...
		for(int idx = local_PMD->autotuner_ctr; idx < REP_TILE; idx++){
			...
			if (predef_controller_hgemm && local_PMD->autotuner[idx]->diff_intialized_params_ATC(predef_controller_hgemm)) 
				local_PMD->autotuner[idx]->mimic_ATC(predef_controller_hgemm);
		...
		else error("PARALiAHgemm: Unsupported OUTPUT_ALGO_MODE = %s\n", OUTPUT_ALGO_MODE);
		...
		for (int i = 0; i < CHL_MEMLOCS; i++) current_SAB[i] = NULL;
		ManageCachesHgemm(local_PMD, curr_autotuner_ctr);
		T = local_PMD->autotuner[curr_autotuner_ctr]->T;
		...
		cpu_timer = csecond();
#endif
		CreateTasksHgemm(local_PMD, curr_autotuner_ctr);
		remaining_tasks = local_PMD->autotuner[curr_autotuner_ctr]->task_num;
		...
		if(buffer_freed) ManageCachesHgemm(local_PMD, curr_autotuner_ctr);
		T = local_PMD->autotuner[curr_autotuner_ctr]->T;
		...
		cpu_timer = csecond();
#endif
		UpdateTasksHgemm(local_PMD);
		remaining_tasks = local_PMD->autotuner[curr_autotuner_ctr]->task_num;
	}
	else error("PARALiAHgemm: Should not reach here - local_PMD->autotuner_ctr = %d\n", local_PMD->autotuner_ctr);
...
#endif
#ifdef PDEBUG
	fprintf(stderr, "PARALiAHgemm(): completed for PMD_cache_entries = %d\n", PMD_cache_entries);
#endif
#ifdef STEST
...
/// A modification of PARALiAHgemm but with given parameters (mainly for performance/debug purposes)
ATC_p PARALiAHgemmControled(char TransA,  char TransB, long int M, long int N, long int K, __half alpha, __half* A, long int ldA,
		__half* B, long int ldB, __half beta, __half* C, long int ldC, ATC_p predef_controller){
	if (predef_controller == NULL){
		warning("Calling PARALiAHgemmControled with empty controller -> falling back to full autotune version \'PARALiAHgemm\'\n");
		return PARALiAHgemm(TransA, TransB,  M, N, K, alpha, A, ldA, B, ldB, beta, C, ldC);
	}
	predef_controller_hgemm = predef_controller;
	return PARALiAHgemm(TransA, TransB,  M, N, K, alpha, A, ldA, B, ldB, beta, C, ldC);
}
