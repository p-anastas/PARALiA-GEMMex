...
///
/// \brief The HGEMM implementation using the new mission-agent-asset C++ classes.
///
...
void ManageCachesHgemm(PMD_p local_PMD){
	for(int loc = 0; loc < CHL_MEMLOCS; loc++) if(local_PMD->autotuner[0]->Block_num[loc]){
		...
		if(current_SAB[loc] == NULL) current_SAB[loc] = 
			...
			error("PARALiAHgemm: PARALiA 3.0 should not enter this\n");
#ifdef DEBUG
...
void CreateTasksHgemm(PMD_p local_PMD){
		...
		__half* B, long int ldB, __half beta, __half* C, long int ldC)
{
	...
	if(!reuse_problem_flag){
		...
		if (predef_controller_hgemm && local_PMD->autotuner[0]->diff_intialized_params_ATC(predef_controller_hgemm))
			local_PMD->autotuner[0]->mimic_ATC(predef_controller_hgemm);
		...
		if(buffer_freed) ManageCachesHgemm(local_PMD);
		T = local_PMD->autotuner[0]->T;
