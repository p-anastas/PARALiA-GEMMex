...
int main(const int argc, const char *argv[]) {
  ...
  long int ldsrc, ldest = ldsrc = TileDim + 1;
	...
	for(short dev_id_idx = 0 ; dev_id_idx < LOC_NUM; dev_id_idx++){
  	...
  	unit_buffs[3*dev_id_idx] = CHLMalloc(TileDim*(TileDim+1)*elemSize, deidxize(dev_id_idx), 1);
		unit_buffs[3*dev_id_idx+1] = CHLMalloc(TileDim*(TileDim+1)*elemSize, deidxize(dev_id_idx), 1);
		unit_buffs[3*dev_id_idx+2] = CHLMalloc(TileDim*(TileDim+1)*elemSize, deidxize(dev_id_idx), 1);
		timer = csecond() - timer;
	...
	for(short dev_id_idx = 0 ; dev_id_idx < LOC_NUM; dev_id_idx++){
		double timer = csecond();
	  CHLVecInit((double*)unit_buffs[3*dev_id_idx], TileDim*(TileDim+1), 42, deidxize(dev_id_idx));
	  CHLVecInit((double*)unit_buffs[3*dev_id_idx+1], TileDim*(TileDim+1), 43, deidxize(dev_id_idx));
		CHLVecInit((double*)unit_buffs[3*dev_id_idx+2], TileDim*(TileDim+1), 44, deidxize(dev_id_idx));
		timer = csecond() - timer;
  ...
  CQueue_p transfer_queue_list[LOC_NUM][LOC_NUM] = {{NULL}};
	...
	CHLMemcpy2DAsync(unit_buffs[3*idxize(loc_dest)], ldest,
	...
	for(short dev_id_idx = 0 ; dev_id_idx < LOC_NUM; dev_id_idx++){
		for(short dev_id_idy = 0 ; dev_id_idy < LOC_NUM; dev_id_idy++){
			...
			for(int itt = 0 ; itt < 10; itt++) CHLMemcpy2DAsync(unit_buffs[3*dev_id_idx+1], ldest,
	...
	for(short dev_id_idk = 0 ; dev_id_idk < LOC_NUM; dev_id_idk++){
			...
			for(int itt = 0 ; itt < 5; itt++) CHLMemcpy2DAsync(unit_buffs[3*dev_id_idx+1], ldest,
										unit_buffs[3*dev_id_idy+1], ldsrc,
										TileDim, TileDim, elemSize,
										deidxize(dev_id_idx), deidxize(dev_id_idy), transfer_queue_list[dev_id_idx][dev_id_idy]);
			for(int itt = 0 ; itt < 5; itt++) CHLMemcpy2DAsync(unit_buffs[3*dev_id_idz+2], ldest,
	...
	for(short dev_id_idx = 0 ; dev_id_idx < LOC_NUM; dev_id_idx++){
		double timer = csecond();
	  CHLFree(unit_buffs[3*dev_id_idx], deidxize(dev_id_idx));
	  CHLFree(unit_buffs[3*dev_id_idx+1], deidxize(dev_id_idx));
		CHLFree(unit_buffs[3*dev_id_idx+2], deidxize(dev_id_idx));
		timer = csecond() - timer;
