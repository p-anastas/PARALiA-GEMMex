...
void Subkernel::prepare_launch(int dev_id){
#ifdef DEBUG
	...
	for (int j = 0; j < TileNum; j++){
		...
		if(tmp->WRP == WR || tmp->WRP == W_REDUCE || tmp->WRP == WR_LAZY) {
			...
			if (tmp->StoreBlock[(dev_id)] &&
				tmp->StoreBlock[(dev_id)]->State != INVALID) tmp->StoreBlock[(dev_id)]->add_writer();
		}
	}
#ifdef DEBUG
	fprintf(stderr, "<-----|\n");
...
void Subkernel::request_data(){
	...
	for (int j = 0; j < TileNum; j++){
		...
		if (tmp->StoreBlock[run_dev_id_idx] == NULL ||
			...
			//if(tmp->WRP == WR){ 
			//	int WB_loc_idx = (tmp->get_initial_location()); 
			...
			if (tmp->WRP == WR || tmp->WRP == RONLY) 
				predef_route[j] = tmp->fetch(tmp->StoreBlock[(run_dev_id)], run_dev_id, predef_route[j]);
		}
...
void STEST_print_SK(kernel_pthread_wrap_p* thread_dev_data_list, double routine_entry_ts, int dev_num)
{
	...
	for (int keri = Sk_num_max - 1; keri >= 0; keri--){
		...
		for (int tileidx = 0; tileidx < thread_dev_data_list[0]->SubkernelListDev[0]->TileNum; tileidx++){
				for (int d = 0; d < dev_num; d++) if(keri < thread_dev_data_list[d]->SubkernelNumDev){
					int Tiledim = thread_dev_data_list[d]->SubkernelListDev[keri]->TileDimlist[tileidx];
...
#endif
					int dev_from = thread_dev_data_list[d]->SubkernelListDev[keri]->dev_in_from[tileidx],
						dev_to = thread_dev_data_list[d]->SubkernelListDev[keri]->dev_in_to[tileidx];
					int is_reader = (Tiledim == 2)? ((Tile2D*) TilePtr)->R_flag : ((Tile1D*) TilePtr)->R_flag;
					int is_writer = (Tiledim == 2)? ((Tile2D*) TilePtr)->W_total : ((Tile1D*) TilePtr)->W_total;
					if (dev_from != -2 && dev_to != -2){
						transfer_map[(dev_to)][(dev_from)]++;
						transfer_map_bw[(dev_to)][(dev_from)]+=
							Gval_per_s(thread_dev_data_list[d]->SubkernelListDev[keri]->bytes_in[tileidx], request_tile_ms[d][tileidx]/1000);
						if (dev_from == -1){
			...
			for (int d = 0; d < dev_num; d++) if(keri < thread_dev_data_list[d]->SubkernelNumDev){
				...
				lprintf(0,"\n");
#endif
			exec_map[(thread_dev_data_list[d]->SubkernelListDev[keri]->run_dev_id)]++;
			exec_map_perf[(thread_dev_data_list[d]->SubkernelListDev[keri]->run_dev_id)]+=Gval_per_s(thread_dev_data_list[d]->SubkernelListDev[keri]->flops, exec_t_ms[d]/1000);
		}
		for (int tileidx = 0; tileidx < thread_dev_data_list[0]->SubkernelListDev[0]->TileNum; tileidx++){
			int Tiledim = thread_dev_data_list[0]->SubkernelListDev[0]->TileDimlist[tileidx];
...
#endif
						int dev_from = thread_dev_data_list[d]->SubkernelListDev[keri]->dev_out_from[tileidx],
